...
st.header('Car Price Prediction ML')
cars_data=pd.read_csv('Cardetails.csv')

cars_data['name']=cars_data['name'].str.split(' ').str[0]
name=st.selectbox('Select the car Brand: ',cars_data['name'].unique())
year=st.slider('Car Manufactured Year: ',min_value=1994,max_value=2024)
km_driven=st.slider('No of Km Driven: ',min_value=1,max_value=200000)
...
if st.button('predict'):
    ...
    st.markdown('Car Price is:  '+str(car_price[0])+' INR  (Approx)')
